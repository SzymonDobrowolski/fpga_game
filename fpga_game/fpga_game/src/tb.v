...
module tb();
  ...
  always@(posedge CLK75_0 or posedge RST) begin
    if (RST) begin
      ...
      // 1. £APIEMY PIKSELE: Kiedy pojawia siê sygna³ wideo, zapisz i przesuñ licznik
      if (VID_DE_d) begin
        if (pixel_addr < SCR_W * SCR_H) begin
          VIDMEM[pixel_addr] <= { &VID_RED, &VID_GREEN, &VID_BLUE };
          
          // Gdy wpadnie OSTATNI piksel (indeks 2399 dla 60x40), ka¿ zapisaæ plik!
          if (pixel_addr == (SCR_W * SCR_H) - 1) begin
            stb_wysw <= 1;
          end
      ...
      // 2. ZAPIS DO PLIKU: Odpala siê u³amek sekundy po ostatnim pikselu
      if (stb_wysw) begin
        file = $fopen("dane.txt", "w+");
        for (i = 0; i < SCR_W*SCR_H; i = i+1) begin
          if (VIDMEM[i] <= 7) $fwrite(file, "%d\n", VIDMEM[i]);
          else $fwrite(file, "x\n");
        end
        $fclose(file);
        
        stb_wysw <= 0;   // Opuszczamy flagê zapisu
        pixel_addr <= 0; // Resetujemy licznik, ¿eby ³apaæ kolejn¹ klatkê gry
      end
